...
export class SetCommand extends Command {
    ...
    public override async messageRun(message: Message, args: Args) {
        ...
        if (message.member?.permissions.has(PermissionFlagsBits.Administrator) === false) {
            if (message.channel.isSendable()) await message.channel.send("You need admin access.");
            return;
        }
        ...
        if (arg1 === "ephemeral-vc") {
            if (
                ...
                if (message.channel.isSendable()) await message.channel.send("I need `Manage Channels` and `Move Members` permissions for that");
                return;
            }
            ...
            if (!channel) {
                if (message.channel.isSendable()) await message.channel.send(`Voice channel \`${arg2}\` not found.`);
                return;
            }
            // joinToCreateVoiceChatManager.set(message.guildId, channel.id);
            await addToJ2CVCManager(message.guildId, channel.id);
            if (message.channel.isSendable()) await message.channel.send(`Voice channel \`${channel.name}\` set to listen to rule \`ephemeral-vc\``);
            return;
        } else {
            if (message.channel.isSendable()) await message.channel.send(`Rule \`${arg1}\` doesn't exist.\nAvailable subcommands : \`ephemeral-vc\``);
            return;
        }
    }
}
